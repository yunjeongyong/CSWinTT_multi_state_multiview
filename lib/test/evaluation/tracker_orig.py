...
class Tracker:
    ...
    def _save_buffer(self, bbox, score, cam: int):

        if cam == 16:
            self.buffers[cam].append(bbox)
            return
        buffer = self.buffers[cam]
        score_buffer = self.score_buffers[cam]
        delta_buffer = self.delta_buffer[cam]
    ...
    def _best_score(self):
        ...
        return self.buffers[6][-(self.buffer_size // 2)]
        ...
        for frame, focal_image, focals, all_frame in focaldataloader:
            ...
            if is_first_frame:
                ...
                for k in range(num_cam):
                    ...
                    self.delta_buffer.append(deque([]))
                self.buffers.append(deque([]))
                for k in range(tracker_num):
                    ...
                    init_state = [x, y, w, h]
                    for i in range(num_cam):
                        tracker[i].initialize(frame[0], _build_init_info(init_state))
                        output_boxes.append(init_state)

            if frame_idx < self.new_frame_idx:
                frame_idx += 1
                continue
            elif self.new_frame_idx == frame_idx:
                for i in range(num_cam):
                    params = self.get_parameters()
                    debug_ = debug
                    if debug is None:
                    ...
                    params.tracker_name = self.name
                    params.param_name = self.parameter_name
                    tracker[i] = self.create_tracker(params)
                    tracker[i].initialize(frame[0], _build_init_info(self.new_bbox))
            ...
            print("first frame_idx_for_plenoptic:", frame_idx_for_plenoptic)
            # camera_overs = 0
            for k in range(tracker_num):
                ...
                for i, f in enumerate(range_focals):
                    ...
                    out = tracker[i].track_multi_state(img, f'{frame_idx}_{ff}')
                    ...
                    if do_plenoptic:
                        ...
                        print("second frame_idx_for_plenoptic:", frame_idx_for_plenoptic)
                ...
                self._save_buffer(bbox_avg, 0, -1)
            ...
            if do_plenoptic and not self.plenoptic_done:
                self.plenoptic_done = True
                best_bbox = self.buffers[16][4]


                print("third frame_idx_for_plenoptic:", frame_idx_for_plenoptic)
                print("buffer-bboxes", self.buffers[6])
                print("buffer-bboxes-17", self.buffers[16])
                # frame_idx_for_plenoptic == 80, plenoptic_before_diff == 5 -> plenoptic_start_num == 75

                plenoptic_start_num = frame_idx_for_plenoptic - self.plenoptic_before_diff
                plenoptic_last_num = frame_idx_for_plenoptic + self.plenoptic_after_diff
                new_bbox, new_frame_idx = external_tracker.run_focal(optional_box=optional_box, debug=debug, save_results=save_results,
                                            start_frame_num=plenoptic_start_num,
                                           last_frame_num=plenoptic_last_num,
                                           bbox=best_bbox, tracker_num=tracker_num)




                print("aaaaa bboxes:", best_bbox, "frame_idx:", frame_idx)


                print("new_bbox:", new_bbox, "new_frame_idx:", new_frame_idx)
                self.new_bbox = new_bbox
                self.new_frame_idx = new_frame_idx
