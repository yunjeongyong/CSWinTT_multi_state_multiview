...
class Transformer_CS(nn.Module):
	...
	def forward(self, template_1, template_2, search, query_embed):
		...
		# (L_all, b , c) , L_all = h2*w2 + h2*w2 + h1*w1 = 64 + 64 + 576 = 704
		...
		merge_pos = torch.cat(
			...
			dim=0)
		# -------------------transformer---------------------
		first_L = self.h2 * self.w2  # template size : 8x8=64

		rel_pos = self.get_relative_position()
		for layer in self.transformer:  # 6번 Transformer 수행
		...
		# -------------------transformer_post---------------------
		query_embed = query_embed.unsqueeze(1).repeat(1, b, 1)  # (N,C) --> (N,1,C) --> (N,B,C) : (1,1,256)
...
class TransformerLayer(nn.Module):
	...
	def forward(self, merge_feat, merge_mask, rel_pos=None, merge_pos=None):
		...
		output = torch.cat(output, -1)  # (704,1,256), 256: 32*8=256
