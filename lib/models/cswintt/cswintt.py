...
class CSWinTT(nn.Module):
	...
	def forward_transformer(self, feat_dict_list, run_box_head=False, run_cls_head=False):
		...
		# feat_dict_list : fixed template + online updated template + search feature
	...
	def forward_box_head(self, merge_feat, hs):
		...
		if self.head_type == "CORNER":
			# ------------adjust shape------------
			hs = hs.permute(1, 2, 0)  # (B, C, N) : 1,256,1
			merge_feat = merge_feat.permute(1, 0, 2) #(B, HW, C) : 1,576,256
			att = torch.matmul(merge_feat, hs)  # (B, HW, N) : 1,576,1
			opt = (merge_feat * att).permute(0, 2, 1).contiguous()  # (B, HW, C) --> (B, C, HW) : 1,256,576
			bs, C, HW = opt.size()
			opt_feat = opt.view(bs, C, self.feat_sz_s, self.feat_sz_s)  # 1,256,24,24
			outputs_coord = box_xyxy_to_cxcywh(self.box_head(opt_feat))  # 1,4
			# ------------return data-----------
			Nq = 1
